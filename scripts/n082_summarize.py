"""N-082 sweep result summarizer.

Reads all results/n082/bench_*.meta.json files, picks the best-accuracy
config per (method, bench), and writes a summary TSV.

Usage:
    python scripts/n082_summarize.py
    python scripts/n082_summarize.py --out results/n082_baseline_sweep_summary.tsv
    python scripts/n082_summarize.py --collapse_threshold 0.51

Collapse criterion: configs with accuracy < collapse_threshold are flagged
as COLLAPSED and excluded from winner selection (matches R-057 framing).
"""
...
def load_meta_files(n082_dir: Path) -> list[dict]:
    rows = []
    for p in sorted(n082_dir.glob("bench_*.meta.json")):
        try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize N-082 VCD/DOLA sweep results")
    parser.add_argument("--n082_dir", default="results/n082",
                        help="Directory containing bench_*.meta.json files")
    parser.add_argument("--out", default="results/n082_baseline_sweep_summary.tsv",
    ...
    n082_dir = Path(args.n082_dir)
    if not n082_dir.exists():
        print(f"Error: {n082_dir} does not exist. Run the sweep first.", file=sys.stderr)
        sys.exit(1)

    rows = load_meta_files(n082_dir)
    if not rows:
        print(f"No bench_*.meta.json files found in {n082_dir}.", file=sys.stderr)
        sys.exit(1)

    print(f"Loaded {len(rows)} meta files from {n082_dir}")
