"""
Smoke test for UniTokHookManager.
...
Mirrors test_hooks_vilau.py but targets UniTok:
  - Projector   : model.model.multi_embedder  (TokenEmbedder, 256 tokens)
  - Decoder     : model.model.layers          (LlamaModel layers)
  - Generate    : model.generate(input_ids, images=vq_code, ...)
  - n_image_tokens : 256 (fixed, VQ codebook)
"""
...
def _test_real_model(model_path: str, tokenizer_path: str) -> None:
    print(f"\nLoading UniTok MLLM from {model_path} ...", flush=True)
    ...
    # Load VQ tokenizer
    print(f"Loading VQ tokenizer from {tokenizer_path} ...", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--model_path",
        default="FoundationVision/unitok_mllm",
        help="Path or HF hub ID of the UniTok MLLM checkpoint.",
    )
    parser.add_argument(
        "--tokenizer_path",
        default="UniTok/checkpoint/unitok_tokenizer.pth",
        help="Path to unitok_tokenizer.pth (the VQ tokenizer checkpoint).",
    )
