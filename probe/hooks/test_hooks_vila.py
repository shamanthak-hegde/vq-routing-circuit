"""
Smoke test for VilaHookManager.
...
Mirrors test_hooks_vilau.py but targets VILA v1.0:
  - Projector: model.model.mm_projector (576 image tokens for VILA-7b)
  - Decoder:   model.model.layers
  - Generate:  model.generate(input_ids, images=...)
"""
...
def _test_real_model(model_path: str) -> None:
    print(f"\nLoading VILA model from {model_path} ...", flush=True)
    ...
    expected_n_img = None  # set from first record's cap.n_image_tokens
    ...
    for rec in [nb, pope_yes, pope_no]:
        ...
        if expected_n_img is None:
            expected_n_img = cap.n_image_tokens
            print(f"  n_image_tokens={expected_n_img}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--model_path",
        default="Efficient-Large-Model/VILA-7b",
        help="Path or HF hub ID of the VILA checkpoint.",
    )
