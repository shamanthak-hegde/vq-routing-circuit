...
Modified for this repo (N-076):
  ...
  - score_chair_from_jsonl() entry point added at the bottom
'''
...
def combine_coco_instances(annotation_path):
    ...
    all_instances = {'info': train_instances['info'],
                     ...
                     'annotations': train_instances['annotations'] + val_instances['annotations']}
...
class CHAIR(object):
    ...
    def get_annotations_from_segments(self):
        instances_path = os.path.join(self.coco_path, 'instances_val2014.json')
        if not os.path.exists(instances_path):
            raise Exception(f"Please download MSCOCO instance annotations to {instances_path}")
        coco_segments = json.load(open(instances_path))
    ...
    def get_annotations_from_captions(self):
        captions_path = os.path.join(self.coco_path, 'captions_val2014.json')
        if not os.path.exists(captions_path):
            raise Exception(f"Please download MSCOCO caption annotations to {captions_path}")
        coco_caps = json.load(open(captions_path))
    ...
    def get_annotations(self):
        ...
        self.coco_ann_info = self.imid_to_objects
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
        ...
        for i in tqdm.trange(len(caps)):
            ...
            cap_dict['metrics'] = {'CHAIRs': 0, 'CHAIRi': 0, 'Recall': 0, 'Len': 0}
        ...
        output['overall_metrics'] = {'CHAIRs': chair_s, 'CHAIRi': chair_i,
                                     'Recall': recall, 'Len': avg_len}
        return output
...
def score_chair_from_jsonl(captions_jsonl: str, coco_path: str) -> dict:
    """Score a JSONL file of captions against COCO val2014 ground truth.

    Each JSONL row must have keys: image_id (int), caption (str).
    Returns the full output dict from compute_chair, including overall_metrics
    with CHAIRs, CHAIRi, Recall, Len.
    """
    evaluator = CHAIR(coco_path)
    return evaluator.compute_chair(captions_jsonl, image_id_key="image_id", caption_key="caption")
...
if __name__ == '__main__':
    ...
    if args.cache and os.path.exists(args.cache):
        ...
        print(f"cache not set or not found, building from scratch...")
