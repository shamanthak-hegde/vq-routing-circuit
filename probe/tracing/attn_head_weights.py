...
def _fallback_non_warn_pope_ids(backend: str) -> list[str]:
    sweep_path = Path(f"results/sweep_{backend}.jsonl")
    if not sweep_path.exists():
        print(f"Warning: {sweep_path} not found; will use first POPE records.")
        return []
    ...
    print(f"Fallback pool: {len(ids)} non-WARN POPE records from {sweep_path}")
    return ids


def _load_records(tokenizer, backend: str, records_from: Path, n_records: int) -> list:
    ...
    if not target_ids:
        target_ids = _fallback_non_warn_pope_ids(backend)

    if target_ids:
        id_set = set(target_ids)
        order = {rid: i for i, rid in enumerate(target_ids)}
        todo = [r for r in records if r.source == "pope" and r.id in id_set]
        todo.sort(key=lambda r: order.get(r.id, 999999))
    else:
        todo = [r for r in records if r.source == "pope"]
...
def _extract_attn_stats(cap, record_id: str, source: str, answer: str) -> dict:
    ...
    if n_heads not in {28, 32}:
        raise AssertionError(f"Unexpected n_heads={n_heads}; expected 28 or 32")
    if seq_q != seq_k:
...
def main() -> None:
    ...
    parser.add_argument("--backend", required=True, choices=["vilau", "unitok"])
