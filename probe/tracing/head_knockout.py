...
def _get_head_dim(layer: Any) -> int:
    """Infer head_dim from o_proj weight shape: (hidden, n_heads * head_dim)."""
...
class SelectiveHeadKnockout:
    ...
    def _pre_hook(self, module: Any, inp: tuple[Any, ...]) -> tuple[Any, ...]:
        x = inp[0]  # (B, seq, n_heads * head_dim)
        hd = self._head_dim
...
def build_intervention(
    mode: str, hm: Any, layer_idx: int, head_idxs: list[int] | str, alpha: float
) -> Any:
...
def select_target_ids(
    ...
    for rid in order:
        ...
        if not rows or rows[0].get("source") != "pope":
            continue
        is_warn = any(r["logit_clean"] <= r["logit_corrupt"] for r in rows)
        if is_warn:
            continue
...
def load_hook_manager(model_path: str) -> tuple[Any, Any]:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="VILA-U self-attention knockout / intervention over the POPE >1.0 subset"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "full_zero is a deprecated alias), selective (per-head zero), "
    ...
    parser.add_argument("--baseline", default="results/sweep_vilau.jsonl")
    ...
    parser.add_argument("--out", default="results/head_knockout_vilau.json")
    ...
    print(f"\nLoading VILA-U model from {args.model_path} ...")
    hm, tokenizer = load_hook_manager(args.model_path)
    ...
    print(
        f"\nIntervention: mode={args.mode}, layer={args.knockout_layer}"
